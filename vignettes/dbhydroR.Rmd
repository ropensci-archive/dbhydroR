...
Water quality data can be retrieved using the `get_wq` function which takes four required arguments. The user must specify a station ID, a test name, and a date range. Station IDs can be located on the [ArcGIS Online Station Map](http://my.sfwmd.gov/WAB/EnvironmentalMonitoring/index.html) or the  [Google Earth kmz file](http://my.sfwmd.gov/KMLEXT/CUSTOMKMLS/DBHydro/DBHydroKML/DBHYDRO_KML.kmz). An abbreviated list of available test names can be found in the [appendix](#appendix) to this document while a full listing can be found at the [DBHYDRO metadata page](http://my.sfwmd.gov/dbhydroplsql/show_dbkey_info.show_data_type_info). Dates must be specified in YYYY-MM-DD format (e.g. 2015-02-26).   The following set of examples retrieve measurements between March 2011 and May 2012. They can be run from the R console by issuing the command:
...
### Further reading
See section on URL-based data access in the [DBHYDRO Browser User's Guide](http://www.sfwmd.gov/portal/page/portal/xrepository/sfwmd_repository_pdf/dbhydrobrowseruserdocumentation.pdf)
